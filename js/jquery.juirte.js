...
$.fn.juirte = function(options){
	...
	* 
	* https://github.com/gcphost/jUIRTE
	* 
	* to-do:
	...
	*/
	...
	return this.each(function(){
       ...
       $this.after(_container); 
		...
		//chris fake new path //.replace(/src="/g, 'src="'+rteSrcAdd)
		var v = $(this).val().replace(/(src=")(?!http?:\/\/)([^"]*)/gi,'$1'+rteSrcAdd+'$2');
		...
		function fnRunDialog(type){
			switch(type){
				...
				break;
				//chris new add-image-dialog using filemanager
				case 'insertImage':
					$('<div/>', {'title': lang.insertimage}).dialog({
						...
						open: function() {
							actRTE = $(this);
							actCommand = fnRunCommand;
						} 
					}).html('<iframe style="width:100%;height:100%;border:0" src="wizards/filemanager/index.php?project='+projectName+'&callback=addImgToRte"></iframe>');
				break;
				/*case 'insertImage':
					$('<div/>', {'title': lang.insertimage}).dialog({
						autoOpen: true,
		...
		function fnSyncContents(){
			...
			// chris 
			$('#'+_id).val(_code.replace(rteSrcReplace, ''));
			hasCanged = true;
